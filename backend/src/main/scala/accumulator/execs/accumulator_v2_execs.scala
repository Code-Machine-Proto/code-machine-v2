...
object accumulator_v2_execs {
  def getHexcode(program: Array[String]): Array[String] = {
    val instructionsArray = accumulator.accumulator_v2.accumulator_v2_compiler
      .getHexCodeFromArray(program);
    instructionsArray
  }
  ...
  def compileAndRun(program: Array[String], id: Int): RunResultsV2 = {
    ...
    chisel3.iotesters.Driver.execute(
      ...
      new accumulator_v2_tester_with_array(DUT, program, id)
    }
...
class accumulator_v2_tester_with_array(
    DUT: accumulator_v2,
    program: Array[String],
    id: Int
) extends PeekPokeTester(DUT) {

  val output_internal_memory = new FileWriter(
    "./output_files/acc_v2_internal_memory_status_" + id + ".txt",
    false
  );
  val output_pc =
    new FileWriter("./output_files/acc_v2_pc_status" + id + ".txt", false);
  val output_acc =
    new FileWriter("./output_files/acc_v2_acc_status" + id + ".txt", false);
  val output_ir =
    new FileWriter("./output_files/acc_v2_ir_status" + id + ".txt", false);
  val output_ma =
    new FileWriter("./output_files/acc_v2_ma_status" + id + ".txt", false);
  val output_state =
    new FileWriter("./output_files/acc_v2_state_status" + id + ".txt", false);
  val output_stimulated_memory = new FileWriter(
    "./output_files/acc_v2_stimulated_memory_status" + id + ".txt",
    false
  );
  val output_stimulated_lines = new FileWriter(
...
//  var instructionsArray = accumulator_v2_compiler.compileFromArray(program)
  ...
  val numberOfInstructions =
    accumulator_v2_compiler.getNumberOfInstructions(program)
  ...
  while (!simulation_ended) {
    for (memIdx <- 0 until DUT.io.InternalMemory.length) {
      if (memIdx < DUT.io.InternalMemory.length - 1) {
        output_internal_memory.write(
          peek(DUT.io.InternalMemory(memIdx)).toInt.toString + ","
        )
      } else {
        output_internal_memory.write(
          peek(DUT.io.InternalMemory(memIdx)).toInt.toString + "\r"
        )
      }
    ...
    output_stimulated_memory.write(
      peek(DUT.io.StimulatedMemoryCell).toString + "\n"
    )
    output_stimulated_memory.flush()

    var stimulatedLines = accumulator_v2_compiler.getStimulatedLines(
...
object accumulator_v2_tester_with_array_exec extends App {
  ...
  for (
    line <- Source
      .fromFile("./programs_files/target_program_v2_shifts.txt")
      .getLines
  ) {
    content = content :+ line
  }
  ...
  chisel3.iotesters.Driver
    .execute(Array("--generate-vcd-output", "on"), () => new accumulator_v2()) {
