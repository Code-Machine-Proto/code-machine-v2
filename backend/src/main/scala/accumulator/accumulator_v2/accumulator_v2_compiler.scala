...
object accumulator_v2_compiler {
      ...
      if (instruction.contains(":")) {
        ...
        if (values.contains(key)) {
          for (labelValue <- values(key)) {
            ...
            if (labelValue < 0) {
              instructionsUInt =
                instructionsUInt :+ intToBinary16(labelValue.toInt).U(16.W)
            } else {
              instructionsUInt = instructionsUInt :+ labelValue.U(16.W)
            }
            instructionsHex =
              instructionsHex :+ "0x" + labelValue.toHexString.toUpperCase
          }
        }
      } else {
        if (instructionsString(index).split(" ").length > 1) {
          ...
          instructionsUInt =
            instructionsUInt :+ (BigInt(opcode) << 8 | BigInt(arg)).U(16.W)
          instructionsHex =
            instructionsHex :+ "0x" + (BigInt(opcode) << 8 | BigInt(
              arg
            )).toInt.toHexString.toUpperCase
        } else {
          // stop or nop
          if (instruction == "stop" || instruction == "nop") {
            instructionsUInt =
              instructionsUInt :+ (BigInt(opcodeMap(instruction)) << 8 | BigInt(
                0
              )).U(16.W)
            instructionsHex = instructionsHex :+ "0x" + (BigInt(
              opcodeMap(instruction)
            ) << 8 | BigInt(0)).toInt.toHexString.toUpperCase
          }
          // Single value (either X or X,)
          else {
            instruction = instruction.trim().replaceAll(",", "")
            instructionsUInt =
              instructionsUInt :+ intToBinary16(instruction.toInt).U(16.W)
            instructionsHex =
              instructionsHex :+ "0x" + (instruction.toInt).toHexString.toUpperCase
          }
  ...
  def compileFromArray(program: Array[String]): Array[UInt] = {
    ...
    instructionsString = removeDirectives(
      formatSpaces(removeEmptyLines(program))
    )
  ...
  def mapLabels(lines: Array[String]): mutable.LinkedHashMap[String, Int] = {
    ...
    for (index <- 0 until lines.length) {
      ...
      if (isEmptyLabel(index, lines))
        nInstructionLabels = nInstructionLabels + 1
  ...
  def mapValues(
      lines: Array[String]
  ): mutable.LinkedHashMap[String, Array[Int]] = {
    var values = mutable.LinkedHashMap[String, Array[Int]]()
    for (line <- lines) {
      if (line.matches(".+:.+")) {
        val label = line.split(":")(0)
  ...
  def computeIndexes(
      instruction: String,
      map: mutable.LinkedHashMap[String, Int]
  ): Int = {
  ...
  def computeOffset(
      key: String,
      map: mutable.LinkedHashMap[String, Array[Int]]
  ): Int = {
    var offset = 0
    if (map.contains(key)) {
      var index = 0
  ...
  def printMap[keyType: ClassTag, valueType: ClassTag](
      map: mutable.LinkedHashMap[keyType, valueType]
  ): Unit = {
    for ((key, value) <- map) {
  ...
  def isEmptyLabel(index: Int, instructions: Array[String]): Boolean = {
    ...
    if (
      index < instructions.length - 1 && !opcodeMap.contains(
        instruction.split(" ")(0)
      )
    ) {
  ...
  def intToBinary16(value: Int): String = {
    ...
    ("b" + binaryString.slice(
      binaryString.length - 17,
  ...
  def getStimulatedLines(
    ...
    if (state == instructionState.fetch) {
      ...
      if (
        instruction == opcode.nop || (instruction == opcode.brz && accValue != 0) || (instruction == opcode.brnz && accValue == 0)
      ) { lineState = lineStates.nop }
      else if (
        instruction == opcode.add || instruction == opcode.sub || instruction == opcode.mul
      ) { lineState = lineStates.add_sub_mul_ex }
      else if (instruction == opcode.adda || instruction == opcode.suba) {
        lineState = lineStates.adda_suba_ex
      } else if (instruction == opcode.addx || instruction == opcode.subx) {
        lineState = lineStates.addx_subx_ex
      ...
      else if (instruction == opcode.lea) { lineState = lineStates.lea_ex }
      else if (instruction == opcode.shl || instruction == opcode.shr) {
        lineState = lineStates.sh_ex
      } else if (
      ...
      } // Verifier si y'a vraiment un branchement
    }
    lineState
  }
