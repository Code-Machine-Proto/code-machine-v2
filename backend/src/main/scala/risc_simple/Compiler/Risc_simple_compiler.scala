...
object asm_compiler {
  ...
  def getHexcodeProgram(program: Array[UInt]): Array[String] = {
    ...
    for (uint <- program) {
      toReturn =
        toReturn :+ "0x" + uint.litValue().toInt.toHexString.toUpperCase
    }
    toReturn
  }
  ...
  def compileFromArray_text(instructionArray: Array[String]): Array[UInt] = {
    ...
    val programLines = removeComments(
      adjustComma(removeEmptyLinesAndSpaces(instructionArray))
    )
    ...
    val dataLabelsMap =
      mapDataLabels(dataLines, textLines.length - textLabelsMap.size)
    val flatenedDataLines = flatDataLines(
      dataLines,
      dataLabelsMap,
      textLabelsMap,
      textLines.length - textLabelsMap.size
    )
    val flatenedTextLines =
      flatTextLines(textLines, dataLabelsMap, textLabelsMap)
  ...
  def compileFromArray(instructionArray: Array[String]): Array[UInt] = {
    var toReturn = Array[UInt]()
    val programLines = removeComments(
      removeEmptyLinesAndSpaces(instructionArray)
    )
  ...
  def mapTextLabels(
    ...
    for (lineIdx <- 0 until textLines.length) {
      if (textLines(lineIdx).contains(":")) {
        toReturn.put(
          textLines(lineIdx).split(":")(0),
          lineIdx - toReturn.size
        ) // next line
      }
    }
    toReturn
  ...
  def mapDataLabels(
    ...
    val toReturn = new scala.collection.mutable.HashMap[String, Int]()
    var nbValues =
      0 // (dataLineStart is the adress where data values will start , right after text without etiquettes)
    for (line <- dataLines) {
      if (line.contains(":")) {
  ...
  def flatDataLines(
      dataLines: Array[String],
      dataLabels: scala.collection.mutable.HashMap[String, Int],
      textLabels: scala.collection.mutable.HashMap[String, Int],
      dataLinesStart: Int
  ): Array[String] = {
    var toReturn = Array[String]()
    ...
    for (line <- dataLines) {
      if (line.contains(":")) {
        val values = line.trim
          ...
          .split(
            ","
          ) // delete etiquette to isolate data values to put into memory
        for (value <- values) {
          toReturn =
            toReturn :+ value // array containing data values sequencetly ( can be with data etiquette or not)
        }
      } else {
    ...
    for (idx <- 0 until toReturn.length) {
      if (
        toReturn(idx).contains("+") && toReturn(idx).trim
          .replaceAll(" +", "")
          .split("\\+")(0) != ""
      ) {
        toReturn(idx) = computeDataValue(
          toReturn(idx),
          "+",
          dataLabels,
          textLabels,
  ...
  def flatTextLines(
    ...
    for (idx <- 0 until toReturn.length) {
      ...
      if (toReturn(idx).contains("+")) {
        toReturn(idx) = computeTextLabelAddress(
          toReturn(idx),
          "+",
          dataLabels,
          textLabels
        ) // return the line with calculated address
      ...
      if (toReturn(idx).trim.replaceAll(" +", " ").split(" ").length > 1) {
        ...
        if (argument.contains(",")) {
          ...
          if (dataLabels.contains(Imm)) {
            toReturn(idx) = op + " " + Reg + "," + dataLabels(
              Imm
            ).toString // it adress from the datalabel hashmap
          }
        }
          ...
          if (dataLabels.contains(argument)) {
            toReturn(idx) = op + " " + dataLabels(
              argument
            ).toString // it adress from the datalabel hashmap
          } else if (textLabels.contains(argument)) {
            toReturn(idx) = op + " " + textLabels(
              argument
            ).toString // it adress from the textlabel hashmap
          }
        }
  ...
  def computeDataValue(
    ...
    var leftOperand =
      line.trim.replaceAll(" +", "").split(operationToCompute)(0)
    var rightOperand =
      line.trim.replaceAll(" +", "").split(operationToCompute)(1)
  ...
  def computeTextLabelAddress(
    ...
    var leftOperand = expressionToCompute(1).trim
      .replaceAll(" +", "")
      .split(operationToCompute)(0)
    var rightOperand = expressionToCompute(1).trim
      .replaceAll(" +", "")
      .split(operationToCompute)(1)
  ...
  def parseStringDataToUInt(data: Array[String]): Array[UInt] = {
    var toReturn = Array[UInt]()
    for (line <- data) {
      toReturn = toReturn :+ intToBinary16(
        Integer.parseInt(line.trim.replaceAll(" +", ""))
      ).U(16.W)
    }
    toReturn
  }
  ...
  def parseStringInstructionToUInt(text: Array[String]): Array[UInt] = {
    ...
    for (line <- text) {
      ...
      // Hex correpondance of operations with arguments
        ...
        val arg =
          line.trim.replaceAll(" +", " ").split(" ", 2)(1) // The argument
        ...
        if (operation == "shr") {
          ...
          val regSrc1 =
            arg.trim.replaceAll(" +", "").split(",")(1).split("r")(1)
        ...
        if (operation == "add") {
          ...
          val regSrc1 =
            arg.trim.replaceAll(" +", "").split(",")(1).split("r")(1)
          val regSrc2 =
            arg.trim.replaceAll(" +", "").split(",")(2).split("r")(1)
    ...
    if (state == instructionState.fetch) {
      ...
      if ((instruction & lastPartMask) == read_mem) {
        ...
        if ((instruction & jTypeMask) == jumpNo) {
          ...
          ((instruction & jTypeMask) == jumpZ) && flagNZ >= flagZ
        ...
        if (
          (instruction & lastPartMask) == opAdd || (instruction & lastPartMask) == opSub ||
          (instruction & lastPartMask) == opAnd || (instruction & lastPartMask) == opOr
        ) {
          lineState = lineStates.opThreeReg
        } else if (
          (instruction & lastPartMask) == opShr || (instruction & lastPartMask) == opShl ||
  ...
  def setStimulatedLines(
      linesToSet: Array[Int],
      lines: Array[String]
  ): Array[String] = {
    val toReturn = lines;
    for (line <- linesToSet) {
