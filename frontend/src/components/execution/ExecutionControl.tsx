...
import { INCREMENT_SIZE_EXECUTION, INCREMENT_SIZE_REGULAR } from "@src/constants/CodeProvider";
/**
      ...
      <div className="flex h-[4rem] items-center gap-5">
        <div className="flex flex-col text-white bg-slate-800 p-3 rounded-md">
          ...
          <select
            ...
            onChange={(e) =>
              dispatch({
                type: CodeAction.CHANGE_MODE,
                mode: e.target.value as PlayerMode,
              })
            }
          >
        ...
        <div className="flex flex-col gap-1">
          <div className="flex items-center">
            ...
            <ExecutionButton onClick={() => dispatch({ type: CodeAction.PLAY_AND_PAUSE })}>
              {processor.isPlaying ? <path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z" /> : <path d="M8 5v14l11-7z" />}
            </ExecutionButton>
          ...
          <input
            type="range"
            className="w-full accent-main-400 cursor-pointer"
            min={0}
            max={maxStep}
            step={processor.mode === PlayerMode.execution ? INCREMENT_SIZE_EXECUTION : INCREMENT_SIZE_REGULAR}
            value={processor.count}
            onChange={(e) => {
              dispatch({
                type: CodeAction.CHANGE_STEP,
                newStep: e.target.valueAsNumber,
              });
        ...
        <div className="flex items-center gap-2">
          ...
          <p className="text-white">Basculer au mode {isVisualMode ? "visuel" : "programmeur"}</p>
        </div>
      </div>
      <a
