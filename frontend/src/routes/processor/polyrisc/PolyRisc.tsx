...
import {
  ProcessorContext,
  DispatchProcessorContext,
} from '@src/components/code/CodeProvider';
...
export default function PolyRiscProcessor() {
  ...
  const [enableInstructionMemory, setEnableInstructionMemory] =
    useState<boolean>(false);

  useEffect(() => {
    dispatch({
      type: CodeAction.CHANGE_PROCESSOR,
      newProcessor: new PolyRisc(),
    });
  }, [dispatch]);

  return isProgrammerMode ? (
    <div className='flex gap-5'>
      <div className='flex flex-col gap-3'>
        ...
        <div className='flex items-center gap-1'>
          <input
            type='checkbox'
            checked={enableInstructionMemory}
            onChange={() =>
              setEnableInstructionMemory(!enableInstructionMemory)
            }
          />
      ...
      {enableRegister && currentStep.regState && (
        <Memory
          ...
          memoryContent={currentStep.regState}
          nom='Registres'
        />
      )}
      ...
      {enableInstructionMemory && currentStep.imState && (
        <Memory
          ...
          memoryContent={currentStep.imState}
          nom="Mémoire d'instructions"
        />
      )}
    </div>
  ) : (
