...
export async function clientAction({
  request,
}: ClientActionFunctionArgs): Promise<{ result: Array<ProcessorStep>; error?: string }> {
  ...
  try {
    ...
    output = '{"imState":[],"steps":[]}';
  }
  const parsed = JSON.parse(output) as { imState: Array<number>; steps: Array<ProcessorStep> };
  const result = parsed.steps.map((step) => ({ ...step, imState: parsed.imState }));
